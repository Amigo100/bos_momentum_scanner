...
SATURDAY (manual)
  └── Copy substack/output/current/newsletter.html to Substack
...
- Backup created in `portfolio/output/portfolio_backups/`
...
- `scanner/output/signals.json` + `scanner/output/archive/YYYY-WXX/signals.json` + `scanner/output/signals.json`
- `scanner/output/analysis_log.csv` (appended)
- `scanner/output/current/report.txt` + legacy `scanner/output/current/report.txt`
- `scanner/output/current/newsletter_briefing.md` + legacy `scanner/output/current/newsletter_briefing.md`
- `twitter/output/grok_prompts/latest_grok_prompts.md`
...
```
Input:  scanner/output/signals.json (stats section)
Output: twitter/output/charts/funnel_YYYYMMDD.png
```
...
```
Input:  scanner/output/signals.json (tickers), portfolio.csv (open positions)
Output: twitter/output/charts/*.png, twitter/output/charts/chart_manifest.json
```
...
```
Input:  scanner/output/current/newsletter_briefing.md, scanner/output/current/market_analysis.md
Output: substack/output/current/newsletter.html, substack/output/current/newsletter.html
```

- `--from-html` mode: Converts Claude.ai HTML output into publication-ready newsletter HTML
...
```
Input:  scanner/output/signals.json, portfolio/output/portfolio.csv, personas/*.yaml, examples/tweet_examples.json
Output: twitter/output/content_queue.json (×3 accounts)
```
...
```
Input:  scanner/output/signals.json, portfolio data
Output: substack/output/current/substack_notes/tuesday_note.md, thursday_note.md
```
...
```
Input:  twitter/output/content_queue.json (×3 accounts)
...
```
Input:  twitter/output/tweet_tracking.json + portfolio P&L
