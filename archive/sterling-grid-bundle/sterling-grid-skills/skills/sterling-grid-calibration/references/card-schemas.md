...
**`portfolio.csv`** (append-only positions ledger; the newsletter's sole price source):
`ticker, entry_date, entry_price, type, conviction, theme, target, rerate_px, overshoot_px, bear_px,
catalyst_window, status (held|exited), exit_date, exit_price, realized_return` — a new Tier-4 BUY appends a
`held` row with its 3c targets; a DD'd Tier-4 sell sets `status=exited` + the exit fields.
