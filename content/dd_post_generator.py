...
# Config imports
from config import TRADES_DIR, get_conviction_text, CURRENCY_SYMBOL
...
# Output paths
try:
    from config.output_paths import ensure_output_structure
...
def load_signals(signals_path: Optional[Path] = None) -> Dict:
    ...
    if OUTPUT_PATHS_AVAILABLE:
        from config.output_paths import get_current_dir
        current = get_current_dir() / "signals.json"
        if current.exists():
            with open(current, "r") as f:
                return json.load(f)

    root = TRADES_DIR / "signals.json"
    if root.exists():
...
def main():
    ...
    for signal in pass_signals:
        ...
        # Optional screenshot
        if args.screenshot:
            png_path = TRADES_DIR / "charts" / f"dd_{ticker}.png"
