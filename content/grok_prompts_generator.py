...
def parse_briefing_markdown(briefing_path: Path) -> PortfolioData:
    ...
    caution_section = re.search(r'### 🟡 (?:CAUTION|CONSIDER) - (?:Watchlist|Wait or Size Down)(.*?)(?=## 📈|## 📋|\Z)', content, re.DOTALL)
    if caution_section:
        # Look for ticker blocks - use \n---\n or \n#### as delimiter
...
def create_watchlist_prompt(signal: Dict, data: PortfolioData) -> GrokPrompt:
    """Create a prompt about a CONSIDER watchlist stock."""
    ticker = signal.get('ticker', 'UNKNOWN')
    theme = signal.get('theme', 'Unknown')
    price = signal.get('price', 0)
    ...
    prompt = f"""SCANNER OUTPUT:
CONSIDER: {ticker} | Theme: {theme} | Price: ${price:.2f}
Reason: {concern_text}
...
def create_newsletter_drop_prompt(data: PortfolioData) -> GrokPrompt:
    ...
    highlights = f"PASS: {', '.join(pass_tickers) if pass_tickers else 'None'} | CONSIDER: {', '.join(caution_tickers) if caution_tickers else 'None'} | Top Theme: {top_theme}"
...
def generate_prompts_markdown(prompts: List[GrokPrompt], data: PortfolioData) -> str:
    ...
    if data.caution_signals:
        lines.append(f"- **CONSIDER Signals:** {', '.join([s['ticker'] for s in data.caution_signals])}")
...
def main() -> None:
    ...
    print(f"     • CONSIDER signals: {len(data.caution_signals)}")
