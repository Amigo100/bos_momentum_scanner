...
try:
    from config import TRADES_DIR, CELEBRATION_THRESHOLDS
except ImportError:
    TRADES_DIR = Path(__file__).resolve().parent.parent / "trades"
    CELEBRATION_THRESHOLDS = [25.0, 50.0, 100.0]

TWEET_TRACKING_FILE = TRADES_DIR / "tweet_tracking.json"
...
def get_unquoted_milestones() -> List[Dict]:
    ...
    """
    try:
        from distribution.signal_tracker import get_open_positions, filter_public_positions
    except ImportError:
        return []
