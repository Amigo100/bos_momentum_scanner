...
def generate_prompts_markdown(prompts: List[GrokPrompt], data: PortfolioData) -> str:
    ...
    lines.append("")
    lines.append("| Day | Slot 1 (Morning) | Slot 2 (Midday) | Slot 3 (Evening) |")
    lines.append("|-----|------------------|-----------------|------------------|")
    ...
    for day in DAYS:
        ...
        for p in sorted(day_prompts, key=lambda x: x.slot):
            slot_labels = {1: "🌅 Morning", 2: "☀️ Midday", 3: "🌆 Evening"}
    ...
    lines.append("5. **Post at optimal times** - Morning (9am), Midday (12pm), Evening (6pm)")
    lines.append("")
    lines.append("**Pro Tips:**")
    lines.append("- Morning posts get more visibility")
    lines.append("- Engagement posts (questions) work best on Sunday")
    lines.append("- Newsletter drops should go Saturday morning")
