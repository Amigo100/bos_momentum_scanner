...
try:
    ...
    from config.output_paths import (
        CONTENT_QUEUE_ACCOUNT2_FILE,
        CONTENT_QUEUE_ACCOUNT3_FILE,
    )
except ImportError:
    _TWITTER_OUTPUT = Path(__file__).resolve().parent.parent / "twitter" / "output"
    CONTENT_QUEUE_FILE = _TWITTER_OUTPUT / "content_queue.json"
    CONTENT_QUEUE_ACCOUNT2_FILE = _TWITTER_OUTPUT / "content_queue_account2.json"
    CONTENT_QUEUE_ACCOUNT3_FILE = _TWITTER_OUTPUT / "content_queue_account3.json"

QUEUES = [
    ("main", CONTENT_QUEUE_FILE),
    ("account2", CONTENT_QUEUE_ACCOUNT2_FILE),
    ("account3", CONTENT_QUEUE_ACCOUNT3_FILE),
]
...
if __name__ == "__main__":
    ...
    all_tweets = {}
    for account, queue_path in QUEUES:
        tweets = load_queue(queue_path)
        if tweets is None:
            print(f"\n⚠️  {account}: Queue file not found ({queue_path})")
            continue
