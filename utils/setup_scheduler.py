...
SCANNER_SCRIPT = SCANNER_DIR / "core" / "scanner.py"
...
def create_wrapper_script() -> Path:
    ...
    SECURITY: The API key is NOT embedded in the script. It relies on the
    shell profile (~/.zshrc or ~/.bash_profile) exporting ANTHROPIC_API_KEY,
...
# Source shell profile to get PATH and environment variables
source {profile} 2>/dev/null || true

# Load .env file if present (fallback for ANTHROPIC_API_KEY)
if [ -f "{env_file}" ]; then
    set -a
    source "{env_file}"
    set +a
...
if [ -z "$ANTHROPIC_API_KEY" ]; then
    echo "ERROR: ANTHROPIC_API_KEY not set. Add it to {profile} or {env_file}" >&2
    exit 1
fi
...
"""
    ...
    # Owner-only permissions (rwx------) — script may reference secrets via env
