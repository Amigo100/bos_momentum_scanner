...
def load_portfolio() -> List[Dict]:
    ...
    with open(portfolio_file, 'r') as f:
        ...
        for row in reader:
            ...
            row['current_price'] = float(row.get('current_price', 0) or row.get('entry_price', 0))
            row['highest_close'] = float(row.get('highest_close', 0) or 0)
            row['pnl_pct'] = 0.0
            if row['entry_price'] > 0:
                row['pnl_pct'] = ((row['current_price'] / row['entry_price']) - 1) * 100
            row['conviction'] = int(row.get('conviction', 3) or 3)
